...
- **tapio-controller**: Cluster coordination binary. Owns the HTTP server side of `tapio-wire/v1`, in-memory agent registry, heartbeat state, read-only status snapshots, event batch sequence-gap tracking, and future Kubernetes-aware coordination.
