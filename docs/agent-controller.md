...
Controller outage must not stop kernel observation or ring buffer consumption. Agents keep the last known valid config, or safe local defaults if no config has ever been fetched. Event batching queues must be bounded; overflow and send failures must be visible. In v0, sink failures are exposed through local metrics and heartbeat counters such as `tapio_sink_drops_total`; heartbeat send failures are logged locally and reported through `tapio_controller_send_failures_total` and later heartbeat counters once delivery resumes.
